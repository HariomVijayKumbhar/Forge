...
logger = logging.getLogger("forge.llm.openrouter")
...
class OpenRouterProvider:
    ...
    async def plan_next_step(
        ...
        if not self.api_key:
            raise ProviderUnavailable("OpenRouter API key is not configured.")

        try:
            from openai import AsyncOpenAI, RateLimitError, AuthenticationError
        except ImportError:
            raise ProviderUnavailable("openai package is not installed. Install with: pip install openai")
        ...
        try:
            ...
            raise RateLimited(f"OpenRouter rate limited: {e}")
        except AuthenticationError as e:
            raise ProviderUnavailable(f"OpenRouter authentication failed: {e}")
        except Exception as e:
            raise ProviderUnavailable(f"OpenRouter API call failed: {e}")
